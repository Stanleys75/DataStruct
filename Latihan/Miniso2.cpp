...
void del(char productID[]){
	int idx = hash(productID);
	
	if(node[idx] != NULL && strcmp(node[idx]->ID,productID)==0){
		char answer;
		do{
		...
		do{
			...
			char answer;
			do{
				printf("Sure to delete %s [Y/N] (Case Incensitive)? : ",&productID);
				scanf("%c",answer);
			}while(answer!='Y' && answer!='y' && answer!='N' && answer!='n');
			...
			if(node[idx]!=NULL){
				data* curr = node[idx];
				data* temp = NULL;
					if(strcmp(node[idx]->ID,productID)==0){
						node[idx]=node[idx]->next;
						free(curr);
						curr-NULL;
						return;
					}
					while(curr!=NULL && strcmp(curr->ID,productID)!=0){
						temp = curr;
						curr = curr->next;
					}
					if(curr!=NULL && strcmp(curr->ID,productID)==0){
						temp->next = curr->next;
						free(curr);
						curr=NULL;
					}
